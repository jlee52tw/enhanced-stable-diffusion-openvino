@echo off
REM Stable Diffusion with OpenVINO Runner
REM This batch file sets up OpenVINO environment and runs the Stable Diffusion script
...
if "%~1"=="" (
    echo Usage: run_diffusion.bat --prompt "your prompt here" [additional options]
    echo.
    echo Example:
    echo   run_diffusion.bat --prompt "A beautiful sunset over mountains" --steps 25
    echo.
    echo Running with default example prompt...
    "%~dp0venv\Scripts\python.exe" stable_diffusion_openvino.py
) else (
    echo Running Stable Diffusion with provided arguments...
    "%~dp0venv\Scripts\python.exe" stable_diffusion_openvino.py %*
)
